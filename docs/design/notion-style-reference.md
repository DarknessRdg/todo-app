> **⚠️ SUPERSEDED (historical).** The app's current visual direction is the
> **"Studio" system** — see `docs/design/system-reference.md` and `CLAUDE.md →
> Design system`. This Notion "warm paper" reference is kept only as history; do not
> apply it to new work.
>
> ---
>
> **Project note.** This file is a *former* registered design reference for
> the app's visual direction — the "warm paper notebook" Notion-style system.
>
> **How it is applied here:** map every value below onto the app's **existing
> semantic tokens** in `src/index.css` (`--background`, `--foreground`, `--card`,
> `--primary`, `--border`, `--ring`, `--muted-foreground`, …) and the shadcn/ui
> component layer. **Do NOT introduce `--color-notion-*` (or any `notion`-named)
> CSS variables into the codebase** — the names below (`--color-notion-blue`,
> `--color-paper-warmth`, etc.) are the *source vendor spec only*, kept verbatim
> for the record. Reusable brand-accent hues that have no semantic slot may be
> registered under neutral names in `@theme` (e.g. `--color-marigold`,
> `--color-coral`) exactly as we already do for accent utilities.
>
> The concise, enforced summary lives in `CLAUDE.md → Design system`. This file is
> the full spec it points at.
...
> **Do not copy these `--color-notion-*` names into `src/index.css`.** They are the
> vendor spec. In this project the values map onto the existing semantic tokens
> (see `CLAUDE.md → Design system` for the mapping table).
